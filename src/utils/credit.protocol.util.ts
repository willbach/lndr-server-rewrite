...
export default class CreditProtocolUtil {
    determineSigner(transaction: CreditRecord, signature: string) {
        const sigAddress = this.signatureToAddress(transaction, signature)
    ...
    signatureToAddress(transaction, signature) {
        const { v, r, s } = this.decomposeSignature(signature)
