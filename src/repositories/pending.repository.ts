...
export default {
    ...
    lookupPendingByAddress: (settlement: boolean, address: string) => {
        if (settlement) {
            ...
            return db.any("SELECT creditor, debtor, pending_credits.amount, memo, submitter, nonce, pending_credits.hash, signature, ucac, settlement_currency FROM pending_credits LEFT JOIN settlements USING(hash) WHERE (creditor = ? OR debtor = ?) AND settlements.hash IS NULL", address)
        }
    },
    ...
    deletePending: (settlement: boolean, hash: string) => {
        if (settlement) {
